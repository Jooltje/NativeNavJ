...
public class SimConnectBindings {
    ...
    private static final MethodHandle SIMCONNECT_CLOSE = LOOKUP.find("SimConnect_Close")
            .map(symbol -> LINKER.downcallHandle(symbol, FunctionDescriptor.of(JAVA_INT, ADDRESS))).orElseThrow();

    private static final MethodHandle SIMCONNECT_ADD_TO_DATA_DEFINITION = LOOKUP.find("SimConnect_AddToDataDefinition")
            .map(symbol -> LINKER.downcallHandle(symbol, FunctionDescriptor.of(JAVA_INT, ADDRESS, JAVA_INT, ADDRESS,
    ...
    private static final MethodHandle SIMCONNECT_REQUEST_DATA_ON_SIM_OBJECT = LOOKUP
            ...
            .map(symbol -> LINKER.downcallHandle(symbol, FunctionDescriptor.of(JAVA_INT, ADDRESS, JAVA_INT, JAVA_INT,
    ...
    private static final MethodHandle SIMCONNECT_CALL_DISPATCH = LOOKUP.find("SimConnect_CallDispatch")
            .map(symbol -> LINKER.downcallHandle(symbol, FunctionDescriptor.of(JAVA_INT, ADDRESS, ADDRESS, ADDRESS)))
            .orElseThrow();

    private static final MethodHandle SIMCONNECT_SET_DATA_ON_SIM_OBJECT = LOOKUP.find("SimConnect_SetDataOnSimObject")
            .map(symbol -> LINKER.downcallHandle(symbol, FunctionDescriptor.of(JAVA_INT, ADDRESS, JAVA_INT, JAVA_INT,
    ...
    private static final MethodHandle SIMCONNECT_TRANSMIT_CLIENT_EVENT = LOOKUP.find("SimConnect_TransmitClientEvent")
            .map(symbol -> LINKER.downcallHandle(symbol, FunctionDescriptor.of(JAVA_INT, ADDRESS, JAVA_INT, JAVA_INT,
    ...
    public static int open(MemorySegment phSimConnect, String appName) throws Throwable {
        try (Arena arena = Arena.ofConfined()) {
            ...
            return (int) SIMCONNECT_OPEN.invokeExact(phSimConnect, szName, MemorySegment.NULL, 0, MemorySegment.NULL,
                    0);
        }
    }
    ...
    public static int addToDataDefinition(MemorySegment hSimConnect, int defineId, String datumName, String unitsName,
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            return (int) SIMCONNECT_ADD_TO_DATA_DEFINITION.invokeExact(hSimConnect, defineId, szDatumName, szUnitsName,
    ...
    public static int requestDataOnSimObject(MemorySegment hSimConnect, int requestId, int defineId, int objectId,
        ...
        return (int) SIMCONNECT_REQUEST_DATA_ON_SIM_OBJECT.invokeExact(hSimConnect, requestId, defineId, objectId,
                period, 0, 0, 0, 0);
    }
    ...
    public static int setDataOnSimObject(MemorySegment hSimConnect, int defineId, int objectId, int flags,
        ...
        return (int) SIMCONNECT_SET_DATA_ON_SIM_OBJECT.invokeExact(hSimConnect, defineId, objectId, flags, arrayCount,
                unitSize, pDataSet);
    }
    ...
    public static int mapClientEventToSimEvent(MemorySegment hSimConnect, int eventId, String eventName)
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            return (int) SIMCONNECT_MAP_CLIENT_EVENT_TO_SIM_EVENT.invokeExact(hSimConnect, eventId, szEventName);
        }
    }

    public static int transmitClientEvent(MemorySegment hSimConnect, int objectId, int eventId, int dwData, int groupId,
            int flags) throws Throwable {
        return (int) SIMCONNECT_TRANSMIT_CLIENT_EVENT.invokeExact(hSimConnect, objectId, eventId, dwData, groupId,
                flags);
    }
